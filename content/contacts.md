...
<b>Endpoint :</b>
`
http://termii.com/api/phonebook/contact/contact_id
...
xhr.open("DELETE", "http://termii.com/api/phonebook/contact/contact_id");
...
var options = {
  'method': 'DELETE',
  'url': 'http://termii.com/api/phonebook/contact/contact_id',
  'headers': {
...
<code-block label="C#" >
  ...
  ```bash
RestClient restClient = new RestClient("http://termii.com/api/phonebook/contact/contact_id");
...
<code-block label="Java" >
  ...
  ```bash
Unirest.setTimeouts(0, 0);
HttpResponse<String> response = Unirest.delete("http://termii.com/api/phonebook/contact/contact_id")
  .header("Content-Type", "application/json")
  .body("{\r\n \"api_key\": \"Your API Key\" }")
  .asString();
  ```
...
curl_setopt_array($curl, array(
  CURLOPT_URL => "http://termii.com/api/phonebook/contact/contact_id",
  CURLOPT_RETURNTRANSFER => true,
  CURLOPT_ENCODING => "",
  CURLOPT_MAXREDIRS => 10,
  CURLOPT_TIMEOUT => 0,
